...
const mockSupabase = {
  ...
  storage: {
    ...
    getPublicUrl: jest.fn().mockReturnValue({ data: { publicUrl: 'https://fake.url/image.jpg' } }),
  },
};
...
const mockOpenAIService = {
    ...
    chat: {
      completions: {
        create: jest.fn().mockResolvedValue({
          choices: [{ message: { content: '[CRITIQUE]: No issues\n[FINAL REPLY]: Critiqued AI reply.' } }],
        }),
      },
    },
  })),
  callOpenAI: jest.fn(async (messages) => {
    ...
    if (Array.isArray(messages) && messages[0]?.role === 'user') {
      ...
      if (typeof content === 'string' && content.includes('invent a short, playful, SFW nickname')) {
        return 'Mocked Nickname';
      }
      if (typeof content === 'string' && content.includes('describe the image')) {
