...
const mockOpenAIServiceInstance_for_generateSuggestions = new OpenAIService(
  "dummy-key-suggestions",
  testModel
);
...
describe("generateSuggestions", () => {
  ...
  it("should call OpenAI with the conversation and parse newline-separated suggestions", async () => {
    ...
    const result = await generateSuggestions(
      sampleConversation,
      mockOpenAIServiceInstance_for_generateSuggestions
    );

    expect(mockCallOpenAI_for_generateSuggestions).toHaveBeenCalledTimes(1);
    expect(mockCallOpenAI_for_generateSuggestions).toHaveBeenCalledWith(
      sampleConversation,
      300
    );
  ...
  it("should return an empty array if OpenAI response is empty or yields no valid suggestions", async () => {
    ...
    let result = await generateSuggestions(
      sampleConversation,
      mockOpenAIServiceInstance_for_generateSuggestions
    );
    expect(result).toEqual([]);
