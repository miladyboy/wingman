...
${originalReply}
  ...
  const res = await openaiClient.chat.completions.create({
    messages: [
      { role: 'system', content: 'You are a quality control agent for AI flirting messages.' },
  ...
  try {
    ...
    if (critiqueMatch && replyMatch) {
      ...
      if (!finalReply) {
        console.warn('[CritiqueAgent] [FINAL REPLY] section is empty. Returning original reply.');
      ...
      if (critIdx !== -1 && replyIdx !== -1) {
        ...
        console.error('[CritiqueAgent] Could not parse critique/final reply. Returning original.');
        critique = 'Format error: Could not parse critique/final reply. Returned original.';
        finalReply = originalReply;
      }
    }
  } catch (err) {
