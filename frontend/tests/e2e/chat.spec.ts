...
test.describe('Core Chat Functionality', () => {
  ...
  test('user can rename a conversation', async ({ page }: { page: any }) => {
    ...
    const chatItem = page.getByTestId('chat-item').filter({ has: page.getByTestId('chat-item-name').filter({ hasText: originalChatTitle }) });
    ...
    await expect(page.getByTestId('chat-item-name').filter({ hasText: originalChatTitle })).not.toBeVisible();
    await expect(page.getByTestId('chat-item-name').filter({ hasText: newChatName })).toBeVisible();
  });

  test('user can delete a single conversation', async ({ page }: { page: any }) => {
...
test.describe('Active chat management', () => {
  ...
  test('keeps last chat active after refresh', async ({ page }: { page: any }) => {
    const { chatTitle: chat1Title } = await createNewChat(page, 1);
    const { chatTitle: chat2Title } = await createNewChat(page, 2);

    await page.getByTestId('chat-item-name').filter({ hasText: chat1Title }).click();
    await page.waitForTimeout(250);
    await expect(
      page.locator('[data-testid="chat-item"][data-active="true"] [data-testid="chat-item-name"]').filter({ hasText: chat1Title })
    ).toBeVisible();
  ...
  test('shows first chat as active on initial load (after setup)', async ({ page }: { page: any }) => {
    const { chatTitle: chat1Title } = await createNewChat(page, 1);
    await expect(page.getByTestId('chat-item').first()).toBeVisible();
    const activeChat = page.locator('[data-testid="chat-item"][data-active="true"]');
    await expect(activeChat).toHaveCount(1);
...
test.describe('Chat ordering', () => {
  ...
  test('sending a message moves chat to top', async ({ page }) => {
    ...
    await expect(page.getByTestId('chat-item-name').first()).toHaveText(chat2Title);
    ...
    await expect(page.getByTestId('chat-item-name').first()).toHaveText(chat1Title);
  });
}); 